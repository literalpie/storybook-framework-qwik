...
`pnpm watch` at the workspace level will build and watch the library.
`pnpm dev` will watch the library, and also build/launch the qwik-app storybook.

After making changes, please run `pnpm test` and `pnpm lint-all` to make sure the build will pass.
